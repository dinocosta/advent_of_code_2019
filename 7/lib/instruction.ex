defmodule Instruction do
  ...
  # OPCODE 3: INPUT
  def run(%{operation: :input}, %Memory{phase_setting: phase_setting} = memory) when phase_setting != nil do
    Memory.update(memory, Memory.at(memory, memory.pointer + 1), phase_setting)
    |> Memory.move_pointer(2)
    |> Map.put(:phase_setting, nil)
  end

  def run(%{operation: :input}, %Memory{input: input} = memory) when input != nil do
    Memory.update(memory, Memory.at(memory, memory.pointer + 1), input)
    |> Memory.move_pointer(2)
    |> Map.put(:input, nil)
  end
